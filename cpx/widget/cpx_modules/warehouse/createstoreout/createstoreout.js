define(function (require, exports, module) {
    ...
    var createstoreout = {
        ...
        initTable: function(data) {
            ...
            var columns = [
                ...
                {display: '出库数量', name: 'mount', width: 100, align: 'right', type: 'numberbox', editor: {type: 'numberbox', precision: 2}},
        ...
        initEvent: function() {
            ...
            $('#create').on('click', function() {
                ...
                if(purchaseDepartment == '') {
                    parent.Public.tips({type: 2, content: '请选择领用部门!'});
                    return false;   
                }
                ...
                $.each(purDes, function(index, obj) {
                    ...
                    if(obj.otherUnitName != '') {//辅助单位是否存在
                        if(obj.mount == '' && obj.otherSurplus == '') {
                            parent.Public.tips({type: 2, content: '第'+obj.id+'出库数量或者辅助单位数量必须填一个!'});    
                            flag = false;
                            return false;
                        }
                    }else {
                        if(!obj.mount || obj.mount == '') {
                            parent.Public.tips({type: 2, content: '第'+obj.id+'出库数量不能为空!'});
                            flag = false;
                            return false;
                        }
                    } 
        ...
        autoCallback: function(value, index, selected, $_dom) {
            var self = createstoreout;
            var grid = createstoreout.grid;
    ...
    exports.init = function() {
        createstoreout.getOriginalData();
    }
});